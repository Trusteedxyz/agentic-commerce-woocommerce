...
final class CartSignalsTest extends TestCase
{
    ...
    {
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('P.O. Box 123', ''));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('PO BOX 7', 'Suite 4'));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('123 Main St', 'PO Box 9'));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('Post Office Box 17', ''));
    }

    public function test_detect_po_box_multi_language(): void
    {
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('Apartado 123', ''));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('Apdo. Postal 47', ''));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('Boîte Postale 5', ''));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('Postfach 9', ''));
        $this->assertTrue(Trusteed_Cart_Signals::detect_po_box('Casella Postale 11', ''));
    }

    public function test_detect_po_box_negatives(): void
    {
        $this->assertFalse(Trusteed_Cart_Signals::detect_po_box('123 Box Hill Rd', ''));
        $this->assertFalse(Trusteed_Cart_Signals::detect_po_box('Posthumous Lane 4', ''));
        $this->assertFalse(Trusteed_Cart_Signals::detect_po_box('', ''));
        $this->assertFalse(Trusteed_Cart_Signals::detect_po_box('456 Oak Ave', 'Apt 2B'));
    }

    public function test_detect_freight_forwarder_known_brands(): void
    {
        $this->assertTrue(Trusteed_Cart_Signals::detect_freight_forwarder('123 Main', '', 'Shipito Inc'));
        $this->assertTrue(Trusteed_Cart_Signals::detect_freight_forwarder('456 Oak', 'MyUS Suite 9', ''));
        $this->assertTrue(Trusteed_Cart_Signals::detect_freight_forwarder('789 Elm', '', 'Stackry LLC'));
    }

    public function test_detect_freight_forwarder_negatives(): void
    {
        $this->assertFalse(Trusteed_Cart_Signals::detect_freight_forwarder('123 Main', '', 'Acme Corp'));
        $this->assertFalse(Trusteed_Cart_Signals::detect_freight_forwarder('', '', ''));
    }
    ...
    public function test_lowest_stock_returns_min_finite(): void
    {
        ...
        $this->assertSame(3, Trusteed_Cart_Signals::lowest_stock($items));
    }
    ...
    public function test_lowest_stock_ignores_null_unmanaged(): void
    {
        ...
        $this->assertSame(5, Trusteed_Cart_Signals::lowest_stock($items));
    }

    public function test_lowest_stock_all_unmanaged_returns_null(): void
    {
        $this->assertNull(Trusteed_Cart_Signals::lowest_stock([['stock' => null], ['stock' => null]]));
        $this->assertNull(Trusteed_Cart_Signals::lowest_stock([]));
    }

    public function test_lowest_stock_zero_is_meaningful(): void
    {
        $items = [['stock' => 10], ['stock' => 0]];
        $this->assertSame(0, Trusteed_Cart_Signals::lowest_stock($items));
    }
    ...
    {
        $this->assertTrue(Trusteed_Cart_Signals::cart_has_subscription([
            ['type' => 'simple'],
            ['type' => 'subscription'],
        ]));
        $this->assertTrue(Trusteed_Cart_Signals::cart_has_subscription([
    ...
    {
        $this->assertFalse(Trusteed_Cart_Signals::cart_has_subscription([['type' => 'simple']]));
        $this->assertFalse(Trusteed_Cart_Signals::cart_has_subscription([]));
    }
    ...
    public function test_stored_value_total_sums_gift_card_lines(): void
    {
        ...
        $this->assertSame(12500, Trusteed_Cart_Signals::stored_value_cents($items));
    }
    ...
    public function test_stored_value_total_zero_when_no_gift_cards(): void
    {
        ...
        $this->assertSame(0, Trusteed_Cart_Signals::stored_value_cents($items));
    }
    ...
    public function test_stored_value_detects_via_tag_or_category(): void
    {
        ...
        $this->assertSame(1500, Trusteed_Cart_Signals::stored_value_cents($items));

        $items2 = [
            ['type' => 'simple', 'price_cents' => 2000, 'qty' => 1, 'categories' => ['gift-cards']],
        ];
        $this->assertSame(2000, Trusteed_Cart_Signals::stored_value_cents($items2));
    }
    ...
    {
        $this->assertTrue(Trusteed_Cart_Signals::is_b2b_order('Acme Corp', null, []));
        $this->assertTrue(Trusteed_Cart_Signals::is_b2b_order('', null, ['b2bking_b2b_customer']));
        $this->assertTrue(Trusteed_Cart_Signals::is_b2b_order('', null, ['wholesale_customer']));
    }

    public function test_b2b_negatives(): void
    {
        $this->assertFalse(Trusteed_Cart_Signals::is_b2b_order('', null, ['customer', 'subscriber']));
        $this->assertFalse(Trusteed_Cart_Signals::is_b2b_order('   ', null, []));
    }
    ...
    public function test_price_delta_bps_zero_when_unchanged(): void
    {
        ...
        $this->assertSame(0, Trusteed_Cart_Signals::max_price_delta_bps($current, $snapshot));
    }
    ...
    public function test_price_delta_bps_picks_max(): void
    {
        ...
        $snapshot = ['100' => 1500, '200' => 2500];
        $this->assertSame(800, Trusteed_Cart_Signals::max_price_delta_bps($current, $snapshot));
    }
    ...
    public function test_price_delta_bps_handles_decrease(): void
    {
        ...
        $snapshot = ['100' => 1500];
        $this->assertSame(3333, Trusteed_Cart_Signals::max_price_delta_bps($current, $snapshot));
    }
    ...
    public function test_verify_price_snap_round_trip(): void
    {
        ...
        $cookie = Trusteed_Cart_Signals::build_price_snap_cookie($prices, $key);
        $this->assertSame($prices, Trusteed_Cart_Signals::verify_price_snap($cookie, $key));
    }

    public function test_verify_price_snap_rejects_tampered(): void
    {
        $key    = bin2hex(random_bytes(32));
        $cookie = Trusteed_Cart_Signals::build_price_snap_cookie(['100' => 1500], $key);
        $decoded = json_decode((string) base64_decode($cookie, true), true);
        $decoded['p']['100'] = 1;
        $tampered = base64_encode((string) json_encode($decoded));
        $this->assertSame([], Trusteed_Cart_Signals::verify_price_snap($tampered, $key));
    }
    ...
    public function test_verify_price_snap_rejects_legacy_unsigned(): void
    {
        ...
        $key    = bin2hex(random_bytes(32));
        $this->assertSame([], Trusteed_Cart_Signals::verify_price_snap($legacy, $key));
    }

    public function test_verify_price_snap_rejects_wrong_key(): void
    {
        $cookie = Trusteed_Cart_Signals::build_price_snap_cookie(['100' => 1500], bin2hex(random_bytes(32)));
        $wrong  = bin2hex(random_bytes(32));
        $this->assertSame([], Trusteed_Cart_Signals::verify_price_snap($cookie, $wrong));
    }
    ...
    public function test_r032_has_blocked_category_hits(): void
    {
        ...
        $this->assertTrue(Trusteed_Cart_Signals::r032_has_blocked_category($items, ['adult', 'weapons']));
    }

    public function test_r032_has_blocked_category_passes_when_no_match(): void
    {
        $items = [['categoryIds' => ['books']]];
        $this->assertFalse(Trusteed_Cart_Signals::r032_has_blocked_category($items, ['adult']));
    }

    public function test_r032_has_blocked_category_empty_blocklist_passes(): void
    {
        $items = [['categoryIds' => ['adult']]];
        $this->assertFalse(Trusteed_Cart_Signals::r032_has_blocked_category($items, []));
    }
    ...
    public function test_r034_has_blocked_sku_hits(): void
    {
        ...
        $this->assertTrue(Trusteed_Cart_Signals::r034_has_blocked_sku($items, ['sku-banned']));
    }

    public function test_r034_has_blocked_sku_passes_when_no_match(): void
    {
        $items = [['id' => 'sku-1']];
        $this->assertFalse(Trusteed_Cart_Signals::r034_has_blocked_sku($items, ['sku-banned']));
    }

    public function test_r034_empty_blocklist_passes(): void
    {
        $items = [['id' => 'sku-1']];
        $this->assertFalse(Trusteed_Cart_Signals::r034_has_blocked_sku($items, []));
    }
    ...
    public function test_r048_digital_good_types_combines_multiple(): void
    {
        ...
        $csv = Trusteed_Cart_Signals::r048_digital_good_types($items);
        $parts = explode(',', $csv);
        sort($parts);
    ...
    public function test_r035_blocks_when_total_exceeds_cap(): void
    {
        ...
        $result = Trusteed_Cart_Signals::evaluateR035MaxOrderValue($cart, ['maxCents' => 10000]);
    ...
    public function test_r035_passes_when_max_cents_missing(): void
    {
        ...
        $result = Trusteed_Cart_Signals::evaluateR035MaxOrderValue($cart, []);
        $this->assertFalse($result['hit']);
    }
    ...
    public function test_r036_passes_on_empty_cart(): void
    {
        $cart = new WC_Cart();
        $cart->items = [];
        $result = Trusteed_Cart_Signals::evaluateR036MaxLineItemValue($cart, ['maxCents' => 100]);
        $this->assertFalse($result['hit']);
    }
    ...
    public function test_r036_passes_when_max_cents_missing(): void
    {
        ...
        $result = Trusteed_Cart_Signals::evaluateR036MaxLineItemValue($cart, []);
        $this->assertFalse($result['hit']);
    }
    ...
    public function test_r036_returns_first_hit(): void
    {
        ...
        $result = Trusteed_Cart_Signals::evaluateR036MaxLineItemValue($cart, ['maxCents' => 10000]);
        $this->assertTrue($result['hit']);
